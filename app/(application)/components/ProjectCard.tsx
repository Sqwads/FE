    ...
    <Link href={`/user-projects/${projectId}`} className="bg-white rounded-lg border border-gray-100 shadow-sm overflow-hidden">
      <div className="relative h-40 overflow-hidden bg-gray-100 flex items-center justify-center">
        {image ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-indigo-50 to-blue-100 transition-transform hover:scale-105 duration-300">
            <span className="text-4xl font-bold text-blue-200 uppercase tracking-widest">{title?.substring(0, 2)}</span>
          </div>
        )}
      </div>
      <div className="p-4">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {tags?.map((tag: any, index: any) => (
            <span
              key={index}
              className={`text-xs px-2 py-1 rounded ${index % 3 === 0 ? 'bg-pink-100 border border-pink-400 text-pink-400' :
